...
//	, t.topic_poster
//-- modify
	$sql = "SELECT f.forum_id, f.forum_name, f.forum_topics, t.topic_poster
...
else
{
	if ( $qtnum > -1 )
	{
		...
		if (($userdata['user_level'] == 1 & $qt_row['admin_auth'] == 0) || ($userdata['user_level'] == 2 & $qt_row['mod_auth'] == 0) || ($userdata['user_level'] == 0 & $qt_row['poster_auth'] == 0) || ($userdata['user_level'] == 0 & $qt_row['poster_auth'] == 1 & $userdata['user_id'] != $topic_row['topic_poster'] ))
		{
...
// crewstyle's mod : Simple Split Topic Type
			if( $board_config['simple_split_topic_type'] )
			{
				...
				{
					if( ($row['topic_type']==POST_GLOBAL_ANNOUNCE) && ($annonce_globale==1) )
					{
						$split_type = $lang['Topic_Global_Announcement'];
						$annonce_globale=0;
					}
					else if( ($row['topic_type']==POST_ANNOUNCE) && ($annonce==1) )
					{
						$split_type = $lang['Topic_Announcement'];
						$annonce=0;
					}
					else if( ($row['topic_type']==POST_STICKY) && ($sticky==1) )
					{
						$split_type = $lang['Topic_Sticky'];
						$sticky=0;
					}
					else if( ($row['topic_type']==POST_NORMAL) && ($sujet==1) )
					{
