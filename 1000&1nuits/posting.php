...
//	, p.post_icon
//-- modify
		$select_sql = (!$submit) ? ', t.topic_title, t.topic_icon, t.topic_description, p.enable_bbcode, p.enable_html, p.enable_smilies, p.enable_sig, p.post_username, pt.post_subject, p.post_icon, pt.post_description, pt.post_text, pt.bbcode_uid, u.username, u.user_id, u.user_sig, u.user_sig_bbcode_uid' : '';
...
//	, $description
//-- modify
			prepare_post($mode, $post_data, $bbcode_on, $html_on, $smilies_on, $error_msg, $username, $bbcode_uid, $subject, $description, $message, $poll_title, $poll_options, $poll_length);
//-- fin mod : post description ------------------------------------------------
...
//	, post_icon
//-- modify
				submit_post($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $username), str_replace("\'", "''", $subject), str_replace("\'", "''", $description), str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length, $post_icon);
//-- fin mod : post description ------------------------------------------------
//-- mod : post icon -----------------------------------------------------------
			}
			break;
...
//-- add
		...
		{
			$template->assign_block_vars('switch_topic_description', array());
		}
//-- fin mod : post description ------------------------------------------------
