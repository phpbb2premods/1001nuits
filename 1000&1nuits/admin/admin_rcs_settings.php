...
* @version $Id: admin_rcs_settings.php,v 0.9 19/09/2006 16:55 reddog Exp $
...
if ( !empty($setmodules) )
{
	...
	$module['Config_rcs_acp']['rcs_a_settings'] = $file;
...
class rcs_settings
{
	...
	function update_style($style_id)
	{
		...
		foreach ( $this->style_fields as $style_field )
		{
			...
			$color[$style_field] = $common->sql_escape_string(stripslashes($updated[$style_field]));
	...
	function _achievement($l_key)
	{
		...
		$message = $lang[$l_key] . '<br /><br />' . sprintf($lang['rcs_click_return_settings'], '<a href="' . append_sid($this->requester . $this->ext) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid('index.' . $this->ext . '?pane=right') . '">', '</a>');
...
class rcs_config extends rcs_settings
{
	...
	function get_options_list()
	{
		return array(
			...
			'rcs_group_stats' => array('blockname' => 'config', 'legend' => 'rcs_group_stats', 'desc' => 'rcs_group_stats_desc', 'var' => 'rcs_group_stats'),
			'rcs_group_admin' => array('blockname' => 'level_groups', 'legend' => 'rcs_group_admin', 'desc' => '', 'var' => 'rcs_group_admin'),
			'rcs_group_mod' => array('blockname' => 'level_groups', 'legend' => 'rcs_group_mod', 'desc' => '', 'var' => 'rcs_group_mod'),
		);
	}
...
$template->assign_vars(array(
	...
	'L_RCS_GROUPS_LEVEL' => $lang['rcs_groups_level'],
	'L_RCS_GROUPS_LEVEL_DESC' => $lang['rcs_groups_level_desc'],
	...
	'I_PICK_COLOR' => $phpbb_root_path . $images['rcs_pick_color'],
