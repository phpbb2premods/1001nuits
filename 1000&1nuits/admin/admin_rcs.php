...
* @version $Id: admin_rcs.php,v 0.13 22/09/2006 20:07 reddog Exp $
...
if ( !empty($setmodules) )
{
	...
	$module['Config_rcs_acp']['rcs_b_manage'] = $file;
...
class rcs_manage
{
	...
	function _achievement($l_key)
	{
		...
		$message = $lang[$l_key] . '<br /><br />' . sprintf($lang['rcs_click_return_manage'], '<a href="' . append_sid($this->requester . $this->ext) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid('index.' . $this->ext . '?pane=right') . '">', '</a>');
		...
		if ( $count_data > 0 )
		{
			...
			foreach ( $this->data as $rcs_id => $rank )
			{
				...
				$template->assign_block_vars('rcs', array(
					...
					'U_RCS_MOVEUP' => append_sid($this->requester . $this->ext . '?mode=moveup&amp;id=' . $rcs_id),
					'U_RCS_MOVEDW' => append_sid($this->requester . $this->ext . '?mode=movedw&amp;id=' . $rcs_id),
					'U_RCS_EDIT' => append_sid($this->requester . $this->ext . '?mode=edit&amp;id=' . $rcs_id),
					'U_RCS_DELETE' => append_sid($this->requester . $this->ext . '?mode=delete&amp;id=' . $rcs_id . '&amp;s=' . $rank['single']),
				));
				$i++;
			}
		}
...
class rcs_details extends rcs_manage
{
	...
	function action()
	{
		...
		if ( $this->mode == 'edit' || $this->mode == 'add' )
		{
			if ( $this->mode == 'edit' )
			{
				...
				$template->assign_block_vars('edit', array(
					'L_RCS_EDIT_TITLE' => sprintf($lang['rcs_edit_title'], $style, $rcs->lng_item($rank['name'])),
			...
			if ( !empty($list_move_after) )
			{
				...
				{
					$items[] = array('name' => $rcs->lng_item($desc), 'value' => $this->data[$val]['order'], 'style' => !empty($val) ? $rcs->get_style($desc, $this->data[$val]['color']) : '', 'selected' => ($val == $after));
				}
			...
			$template->assign_vars(array(
				...
				'I_PICK_COLOR' => $this->root . $images['rcs_pick_color'],

				'S_RCS_ACTION' => append_sid($this->requester . $this->ext),
			));
		...
		else if ( $this->mode == 'save' )
		{
			...
			// prepare data
			$rcs_name = $common->_smart($rcs_name);
			$rcs_color = stripslashes($rcs_color);
			$rcs_color = $common->sql_escape_string($rcs_color);
			$rcs_single = $common->_smart($rcs_single);
			$rcs_display = $common->_smart($rcs_display);
			$rcs_order = $common->_smart($rcs_order);
		...
		{
			$rcs_tmp_single = request_var('s', TYPE_INT);
			...
			else
			{
				...
				$template->assign_vars(array(
					...
					'S_CONFIRM_ACTION' => append_sid($this->requester . $this->ext),
				));
...
$template->assign_vars(array(
	...
	'U_RCS_CREATE' => append_sid($requester . $phpEx . '?mode=add'),

	'S_RCS_ACTION' => append_sid($requester . $phpEx),
));
