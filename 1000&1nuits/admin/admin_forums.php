...
// Added by Attached Forums MOD
				...
				else
				{
					...
					{
						$attachable_forums = '<option value = "-1"'.($forum_attached_id==-1?' selected':'').'>' . $lang['Attached_forum_choose'] . '</a>';
...
// Added by Attached Forums MOD
				...
				if( $db->sql_numrows($result1) > 0 )
				{
					$attachable_forums = '<option value = "-1"'.(($forum_attached_id==-1 || !$forum_attached_id)?' selected':'').'>' . $lang['Attached_forum_not_attached'] . '</a>';
					while( $row1 = $db->sql_fetchrow($result1) )
					{
...
//-- modify
			$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, cat_id, attached_forum_id, forum_desc, forum_order, forum_status, forum_link, forum_link_date, forum_qpes, forum_icon, prune_enable, points_disabled" . $field_sql . ")
		...
		for($j = 0; $j < $total_forums; $j++)
		{
			...
			if ($forum_rows[$j]['cat_id'] == $cat_id)
			{
				...
				if ($forum_rows[$j]['attached_forum_id'] !=-1)
				{
					...
					if ($forum_rows[$j]['attached_forum_id']==$forum_id)
					{
						//attached to itself
						$ok=FALSE;
					}
					if ($forum_rows[$j]['attached_forum_id']==0)
					{
						//invalid parent
						$ok=FALSE;
					}
				...
				if ($do_template)
				{
					...
					for($k = 0; $k < $total_forums; $k++)
					{
						...
						if ($subforum_rows[$k]['attached_forum_id'] == $forum_id)
						{
							...
							$subforum_link = ( $subforum_rows[$k]['forum_link']=='' ) ? append_sid($phpbb_root_path."viewforum.$phpEx?" . POST_FORUM_URL . "=$subforum_id") : append_sid($phpbb_root_path."forumlink.$phpEx?" . POST_FORUM_URL . "=$subforum_id");
							$subforum_target =( $subforum_rows[$k]['forum_link']=='' ) ? '' : 'target="_blank"';
							...
							$template->assign_block_vars("catrow.forumrow",	array(
								...
								'FORUM_ICON_IMG' => ($subforum_rows[$k]['forum_icon']) ? '<img src="' . $phpbb_root_path . $subforum_rows[$k]['forum_icon'] . '" alt="'.$subforum_rows[$k]['forum_name'].'" title="'.$subforum_rows[$k]['forum_name'].'" />' : '',
