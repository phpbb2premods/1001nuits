...
//	, &$description
//-- modify
function prepare_post(&$mode, &$post_data, &$bbcode_on, &$html_on, &$smilies_on, &$error_msg, &$username, &$bbcode_uid, &$subject, &$description, &$message, &$poll_title, &$poll_options, &$poll_length)
//-- fin mod : post description ------------------------------------------------
{
	global $board_config, $userdata, $lang, $phpEx, $phpbb_root_path;

	// Check username
	if (empty($username) && !$userdata['session_logged_in'])
	{
		$error_msg .= (!empty($error_msg)) ? '<br />' . $lang['Empty_name'] : $lang['Empty_name'];
	}
...
//	, $post_icon = 0
//-- modify
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, $post_username, $post_subject, $post_description, $post_message, $poll_title, &$poll_options, &$poll_length, $post_icon = 0)
//-- fin mod : post description ------------------------------------------------
//-- fin mod : post icon -------------------------------------------------------
{
...
//-- modify
		$sql  = ($mode != "editpost") ? "INSERT INTO " . TOPICS_TABLE . " (topic_title, topic_description, topic_poster, topic_time, forum_id, topic_status, topic_type, topic_icon, topic_vote) VALUES ('$post_subject', '$post_description', " . $userdata['user_id'] . ", $current_time, $forum_id, " . TOPIC_UNLOCKED . ", $topic_type, $post_icon, $topic_vote)" : "UPDATE " . TOPICS_TABLE . " SET topic_title = '$post_subject', topic_description = '$post_description', topic_type = $topic_type, topic_icon=$post_icon " . (($post_data['edit_vote'] || !empty($poll_title)) ? ", topic_vote = " . $topic_vote : "") . " WHERE topic_id = $topic_id";
//-- fin mod : post description ------------------------------------------------
//-- fin mod : post icon -------------------------------------------------------
		if (!$db->sql_query($sql))
		{
...
//-- modify
	$sql = ($mode != 'editpost') ? "INSERT INTO " . POSTS_TEXT_TABLE . " (post_id, post_subject, post_description, bbcode_uid, post_text) VALUES ($post_id, '$post_subject', '$post_description', '$bbcode_uid', '$post_message')" : "UPDATE " . POSTS_TEXT_TABLE . " SET post_text = '$post_message',  bbcode_uid = '$bbcode_uid', post_subject = '$post_subject', post_description = '$post_description' WHERE post_id = $post_id";
//-- fin mod : post description ------------------------------------------------	
	if (!$db->sql_query($sql))
	{
		message_die(GENERAL_ERROR, 'Error in posting', '', __LINE__, __FILE__, $sql);
...
//-- modify
	add_search_words('single', $post_id, stripslashes($post_message), stripslashes($post_subject), stripslashes($post_description));
//-- fin mod : post description ------------------------------------------------
