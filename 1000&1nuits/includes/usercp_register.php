<?php
//------------------- Mod : Advanced Visual Confirmation 1.1.1
/***************************************************************************
 *                            usercp_register.php
 *                            -------------------
...
// End Account Self-Delete MOD
	...
	$strip_var_list = array('email' => 'email', 'icq' => 'icq', 'aim' => 'aim', 'msn' => 'msn', 'skype' => 'skype', 'yim' => 'yim', 'website' => 'website', 'location' => 'location', 'occupation' => 'occupation', 'interests' => 'interests', 'confirm_code' => 'confirm_code');
...
// End add - Gender MOD
	...
	if ( $mode == 'register' )
	{
		...
		$user_absent = ( isset($HTTP_POST_VARS['user_absent']) ) ? ( ($HTTP_POST_VARS['user_absent']) ? TRUE : 0 ) : 0;
	}
	else
	{
		...
		$user_absent = ( isset($HTTP_POST_VARS['user_absent']) ) ? ( ($HTTP_POST_VARS['user_absent']) ? TRUE : 0 ) : $userdata['user_absent'];
	}
...
//-- modify
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) ."', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_birthday = '" . $user_birthday . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_sig_bbcode_uid = '$signature_bbcode_uid', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_skype = '" . str_replace("\'", "''", $skype) . "', user_attachsig = $attachsig, user_qp_settings = '" . $user_qp_settings . "', user_allowsmile = $allowsmilies, user_absent = $user_absent, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_notify_donation = $notifydonation, user_popup_pm = $popup_pm, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active, user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . ", user_gender = '$gender'
				WHERE user_id = $user_id";
//-- fin mod : quick post es ---------------------------------------------------
//-- fin mod : birthday --------------------------------------------------------
...
//	, '" . str_replace("\'", "''", $flag) . "'
//-- modify
			$sql = "INSERT INTO " . USERS_TABLE . "	(user_id, username, user_regdate, user_password, user_email, user_icq, user_website, user_occ, user_from, user_interests, user_flag, user_birthday, user_browser, user_sig, user_qp_settings, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_skype, user_attachsig, user_allowsmile, user_absent, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_notify_donation, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_gender, user_level, user_allow_pm, user_active, user_actkey)
				VALUES ($user_id, '" . str_replace("\'", "''", $username) . "', " . time() . ", '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $flag) . "', '" . $user_birthday . "', '" . str_replace("\'", "''", $browser) . "', '" . str_replace("\'", "''", $signature) . "', '" . $user_qp_settings . "', '$signature_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', '" . str_replace("\'", "''", $skype) . "', $attachsig, $allowsmilies, $user_absent, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $notifydonation, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, '$gender', 0, 1, ";
//-- fin mod : flags -----------------------------------------------------------
//-- fin mod : quick post es ---------------------------------------------------
//-- fin mod : browsers list ---------------------------------------------------
//-- fin mod : birthday --------------------------------------------------------
...
//-- fin mod : browsers list ---------------------------------------------------
	...
	$user_absent = $userdata['user_absent'];
...
//	, $flag
//-- modify
	display_avatar_gallery($mode, $avatar_category, $user_id, $email, $current_email, $coppa, $username, $email, $new_password, $cur_password, $password_confirm, $icq, $aim, $msn, $skype, $yim, $website, $location, $occupation, $interests, $flag, $bday_date, $browser, $signature, $viewemail, $notifypm, $popup_pm, $notifyreply, $attachsig, $allowhtml, $allowbbcode, $allowsmilies, $user_absent, $allowviewonline, $user_style, $user_lang, $user_timezone, $user_dateformat, $userdata['session_id'], $user_qp_settings, $gender);
//-- fin mod : birthday --------------------------------------------------------
//-- fin mod : browsers list ---------------------------------------------------
//-- fin mod : quick post es ---------------------------------------------------
//-- fin mod : flags -----------------------------------------------------------
...
// User's Data
		...
		'USER_ABSENT_YES' => ( $user_absent ) ? 'checked="checked"' : '',
		'USER_ABSENT_NO' => ( !$user_absent ) ? 'checked="checked"' : '',		
...
// User's Lang
		...
		'L_USER_ABSENT' => $lang['Absent_user_avatar'],
		'L_USER_ABSENT_EXPLAIN' => $lang['Absent_user_avatar_Explain'],		
...
// PréMOD ~ 1000 & 1 nuits ~
	...
	// Gender
		...
		'I_GENDER_MALE' => '<img src="' . $images['icon_minigender_male'] . '" alt="' . $lang['Gender'] .  ' : ' . $lang['Male'] . '" title="' . $lang['Gender'] . ' : ' . $lang['Male'] . '" border="0" />',
		'I_GENDER_FEMALE' => '<img src="' . $images['icon_minigender_female'] . '" alt="' . $lang['Gender'] . ' : ' . $lang['Female'] . '" title="' . $lang['Gender'] . ' : ' . $lang['Female']. '" border="0" />',
