...
//	, pt.post_description
//-- modify	
			$sql = "SELECT pt.post_text, pt.bbcode_uid, pt.post_subject, pt.post_description, p.*, f.forum_id, f.forum_name, t.*, u.username, u.user_id, u.user_sig, u.user_sig_bbcode_uid  
...
//-- add
				if ( $post_description )
				{
					$template->assign_block_vars('searchresults.switch_post_description', array());
				}
//-- fin mod : post description ------------------------------------------------
...
//-- add
				...
				{
					$template->assign_block_vars('searchresults.switch_topic_description', array());
				}
//-- fin mod : post description ------------------------------------------------
...
// Search forum
//
$sql = "SELECT c.cat_title, c.cat_id, f.forum_name, f.forum_id, f.forum_link  
...
while( $row = $db->sql_fetchrow($result) )
{
	...
	{
		$s_forums .= ( $row['forum_link']=='' ) ? '<option value="' . $row['forum_id'] . '">' . $row['forum_name'] . '</option>' : '';
